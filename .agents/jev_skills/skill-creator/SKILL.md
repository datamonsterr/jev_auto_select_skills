...
### Assess Skill Placement & Move to Jev (`move-to-jev`)

After creating or updating a skill, run `assess.ts` to determine if the skill should be an always-on normal skill or moved to the Jev skill bank:

```bash
bun run scripts/assess.ts <path/to/skill-folder> --move
```

**Assessment Rules:**
1. **Always-Use Skills (`ALWAYS_ON`)**:
   - Universal persona/tone modifiers (e.g., `caveman` terse tone, persona enhancers, core system discipline rules).
   - **Destination**: Kept in normal skills directory (`.agents/skills/` or `~/.agents/skills/`) to load directly into the LLM context on every prompt.
2. **Optional / Task-Specific Skills (`JEV_ROUTED`)**:
   - Specialized tools, frameworks, and workflows (e.g., React, TDD, Docker, Supabase, Git rebase, SRS writing).
   - **Destination**: Moved to the Jev skills bank (`.agents/jev_skills/` or `~/.agents/jev_skills/`).
   - Jev evaluates incoming prompts in ~400ms and dynamically injects the skill instructions only when relevant, preventing context bloat.
